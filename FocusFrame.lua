...
function ALUF_FocusFrame:Initialise()
	...
	local scale = ArenaLiveCore:GetDBEntry(addonName, "FocusFrame/Scale"); 
	scale = scale / 100; 
...
function ALUF_FocusFrame:Toggle()
...
function ALUF_FocusFrame:UpdateAttachements()
	
	local castBarEnabled = ArenaLiveCore:GetDBEntry("ArenaLiveUnitFrames", "FocusFrame/CastBar/Enabled");
	local castBarPosition = ArenaLiveCore:GetDBEntry("ArenaLiveUnitFrames", "FocusFrame/CastBar/Position");
	local auraFrameEnabled = ArenaLiveCore:GetDBEntry("ArenaLiveUnitFrames", "FocusFrame/AuraFrame/Enabled");
	local auraFramePosition = ArenaLiveCore:GetDBEntry("ArenaLiveUnitFrames", "FocusFrame/AuraFrame/Position");
	...
	if ( auraFramePosition == "BELOW" ) then
		...
		ArenaLiveCore:SetDBEntry("ArenaLiveUnitFrames", "FocusFrame/AuraFrame/GrowUpwards", false);
	elseif ( auraFramePosition == "ABOVE" ) then
		self.auraFrame:SetPoint("BOTTOMRIGHT", self, "TOPRIGHT", -33, 2);
		ArenaLiveCore:SetDBEntry("ArenaLiveUnitFrames", "FocusFrame/AuraFrame/GrowUpwards", true);
	end	
end